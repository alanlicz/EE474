...
int check1() {
  ...
  while (GPTMRIS_1 == 0x0 && switch_input('O') == 0x8){} //delay while timer is counting down
  if (GPTMRIS_1 == 0x1 && switch_input('O') == 0x8) {
    return 1;
  } else {
    return 0;
  } */
...
int check2() {
  ...
  while (GPTMRIS_2 == 0x0 && switch_input('P') == 0x4){} //delay while timer is counting down
  if (GPTMRIS_2 == 0x1 && switch_input('P') == 0x4) {
    return 1;
  } else {
    return 0;
  } 
...
void trafficLight_SM()
{
  ...
  switch(state) {   // Transitions
     case off:  // Initial transition  
       //if (switch_input('O') == 0x8 && ButtonReleased == 1){ //if On/Off button is pressed and was previously unpressed
       if (check1()){ //if On/Off button is pressed and was previously unpressed
         state = stop;
       } else {
         state = off;
       }
     ...
     case go:
        ...
        if (check1()) {//if On/Off button is pressed and was previously unpressed
           state = off;
        } else if (switch_input('P') == 0x4) {//if Pedestrian button is pressed
           state = warn;
        } else {
          state = stop;
        }
  ...
  switch(state) {   // State actions
     case off:
        /*
        ButtonReleased = 0;//Indicate On/Off button not is currently pressed
        if (switch_input('O') == 0x0){
          ButtonReleased = 1;//Indicate On/Off button is currently pressed
        }*/
        //wait(1);
        //GPTMCTL_0 &= ~0x1; //disable timer 0 //used to be == 0x0
        //GPTMTAILR_0 = 0x1E84800; //32,000,000 in HEX to achieve a 1 Hz blink rate
        //GPTMCTL_0 |= 0x1; //enabling timer 0 
        LED_off('A');
        wait(5);
