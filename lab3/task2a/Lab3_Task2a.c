...
* This file contains main function for task2 part a
*/
...
int main(void) {
  ...
  while(1) {
    ...
    if ((GPIODATA_J & 0x2) == 0x0){ //120
      freq = 120;
      PLL_Init(freq);
      printf("freq = 120");
    }
    if ((GPIODATA_J & 0x1) == 0x0){ //120
      freq = 12;
      PLL_Init(freq);
      printf("freq = 12");
    }
   
    temp = 147.5 - ((75 * (3.3) * ADC_value) / 4096); //<-- this is not right, can give large negative values
    printf("Temp: %f\n", temp);
    printputty();
  }
  return 0;
